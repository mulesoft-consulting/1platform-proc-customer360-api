pipeline {
  ...
  stages {
    stage('Prepare') {
      steps {
        configFileProvider([configFile(fileId: "${BRANCH_NAME}-proc-customer360-api.yaml", replaceTokens: true, targetLocation: './src/main/resources/config/configuration.yaml')]) {
 ...
 withMaven(
    ...
    stage('Deploy Production') {
        ...
        environment {
          ...
          APP_NAME = 'nto-customer360-api-v2'
        }
        steps {
          withMaven(
            mavenSettingsConfig: 'f007350a-b1d5-44a8-9757-07c22cd2a360'){
